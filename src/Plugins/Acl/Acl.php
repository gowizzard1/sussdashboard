...
namespace Siwebapp\Plugins\Acl;
...
{
    const APC_CACHE_VARIABLE_KEY = 'siwebapp-acl';
    ...
    public function rebuild(): AclMemory
    {
        ...
        foreach ($profiles as $profile) {
            $acl->addRole(new AclRole($profile->role_name));
        }
        ...
        foreach ($profiles as $profile) {
            ...
            foreach ($profile->getPermissions() as $permission) {
                $acl->allow($profile->role_name, $permission->resource, $permission->action);
            }

            // Always grant these permissions
            $acl->allow($profile->role_name, 'authorizedUsers', 'changePassword');
        }
